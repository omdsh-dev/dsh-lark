...
| `/cd <名称或路径>` | 切换工作区 |
| `/get <路径>` | 把工作区里的文件发到聊天 |
| `/put <路径> [--into <文档链接>]` | 把工作区产物新建为飞书文档，或追加到指定文档 |
| `/model` | 打开模型选择卡片 |
...
- 出站文件的路径一律只说“工作区内的相对位置”，宿主绝对前缀不会出现在任何一句给人或给模型看的话里——包括读取失败时文件系统自己的报错（`/get` 的回复和 `send_file` 给模型的报错）。失败分支恰恰是提示注入能主动触发的那条。
- 一个群同时最多有 3 个持有内容的 Agent 出站操作，`send_file` 与 `send_doc` 共用这个额度：审批前要先冻结实际发送内容，所以数量必须有上限。第 4 次会在读取内容前被拒绝并告诉模型等前面几个先有结果。这个数字不可配置，调大等于同时买回内存风险和审批疲劳。
- `receiveDocs` 默认开：聊天正文里的 docx / Wiki 链接会在读取当下落成 Markdown 快照，与普通入站文件共用 `.dsh-lark/inbox/<时间戳>-<消息哈希>/`；每条消息最多读取 3 篇，单篇复用 `maxReceiveFileBytes`。旧版文档、表格、多维表格、妙记和云盘文件不会伪装成已读取，Agent 会收到明确注记。
- `sendDocs` 默认开：Agent 可用 `send_doc` 把工作区 Markdown 产物写成飞书文档；私聊新建后只加发起人，群聊新建必须先审批、允许后才创建并加当前群。`send_doc into=…` 只能追加到本会话读过的文档；人明确输入的 `/put --into` 不受这项约束。追加已有文档不会改变原有权限。
- 新建文档成功但添加协作者失败时，渠道仍会回真实链接，并说明这篇文档只有机器人能打开、你无法自行共享它（共享的前提是先能打开），可在权限恢复后用工作区里同样的内容重新发布；文档 token 同时写入操作台，这是运维处理“谁都打不开的文档”的唯一入口。不会把已经存在的文档伪装成完全成功或完全失败。
- 一轮被中途取消时，已经写进飞书的内容不会被说成失败：渠道照实回链接，并注明“这一轮已取消，但写入已经生效，不要重发”，同时写入操作台。请求还在路上时被取消，则明说平台是否已生效未知、重发前请先确认。把已生效的追加报成失败，人就会重发，同一段内容会被追加两遍。
- 文档能力启动时通过 `application.v6.scope.list` 独立探测：`receiveDocs`、`sendDocs`、`commentDocs` 默认开。探测被拒或超时时（以及 scope 名单尚未经真实租户验证时），三项能力**乐观点亮**：一份抄错的名单若直接关灯，就再也发不出那个唯一能纠正它的请求。能关灯的只有两件事——一份已验证的名单显示缺 scope，或一次真实的 `permission_violations`。`/status` 会如实显示读、写、评论能力及缺失 scope。
- 真实文档调用若返回 `permission_violations`，下一次 Agent 创建时会收起对应工具。`docAuthorizeOnDemand` 默认开：授权链接优先私聊发给扫码注册者；没有注册者记录时才退回原聊天，并明确警示该链接可以修改应用配置。扫码后会重新探测，`addons` 未灰度或权限仍在审批时只报告“权限未变化”，不会声称授权成功。
- `commentDocs` 默认开：`read_doc_anchors` 只为本会话已读文档在原快照旁落一份带评论与 block id 的 `<标题>.blocks.xml`；重复读取只通过首次记录并再次核验身份的同一文件句柄刷新同一路径，大小复用 `maxReceiveFileBytes`。这里优先保证父目录换链或同大小替换不能把内容导向别处，因此刷新不承诺原子替换。`comment_on_doc` 也只接受这些文档并且始终携带锚点，不会创建全文评论。评论不逐条审批；单条文本按 Unicode code point 合计最多 10000，`maxDocCommentsPerTurn` 默认 20 且在下一轮重置。配置开关与文档评论能力任一关闭时，这两个工具都不会向 Agent 注册，并会进入 presence 的 unavailable 列表。
