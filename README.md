...
- 新建文档成功但添加协作者失败时，渠道仍会回真实链接并明确提示“只有机器人能打开，请手动共享”，同时写入操作台；不会把已经存在的文档伪装成完全成功或完全失败。
- 文档能力启动时通过 `application.v6.scope.list` 独立探测：`receiveDocs`、`sendDocs`、`commentDocs` 默认开，但只有对应 tenant scopes 全部授予后才点亮；`/status` 会如实显示读、写、评论能力及缺失 scope。
