...
- `commentDocs` 默认开：文档评论中有人 @ 机器人时，渠道按文档派生会话，把引文、提问、文档链接和不可信数据提示合成入站消息，并把终态回复写回触发线程。首次开会话会私聊通知注册者，公开可见文档会附风险提醒；最多保持 100 篇活跃文档，超出后释放最久未用的 Agent，但不删除持久会话。渠道不注册 `read_doc_anchors` / `comment_on_doc`；超过 10000 个 Unicode code point 的回复会在发送前截断并带可见说明。
