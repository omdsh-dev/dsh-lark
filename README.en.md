...
- `receiveDocs` is on by default: docx and Wiki links in the message body become point-in-time Markdown snapshots in the same `.dsh-lark/inbox/<timestamp>-<message hash>/` used by ordinary inbound files. One message reads at most three documents, and each reuses `maxReceiveFileBytes`; legacy docs, sheets, Base, Minutes, and Drive files instead leave an explicit note for the agent.
- `sendDocs` is on by default: `send_doc` turns a workspace Markdown artifact into a Feishu document. A new document in a direct chat grants only the initiating user; in a group it is not created until the group approves, then grants that group. `send_doc into=…` may append only to a document this conversation has read, while a human's explicit `/put --into` target is unrestricted. Appending never changes the existing document's audience.
...
- Document capabilities are probed independently at startup through `application.v6.scope.list`. `receiveDocs`, `sendDocs`, and `commentDocs` default on, and when the probe is refused or times out — or while a scope list has not been verified against a real tenant — all three light OPTIMISTICALLY: a list copied out of documentation that turned a capability off would stop the very call whose refusal is the only thing that could correct it. Exactly two things take a capability away: a verified list showing a missing scope, or a real `permission_violations` from the platform. `/status` shows read, write, comment, and exact missing scopes.
- When a real document call returns `permission_violations`, the corresponding tools disappear from the next agent created. `docAuthorizeOnDemand` defaults on: the app-changing authorization link is sent privately to the recorded registrar, falling back to the source chat only with an explicit warning. A post-scan probe reports what actually changed; ignored `addons` or pending approval is never presented as success.
- `commentDocs` defaults on. `read_doc_anchors` creates a comment-aware `<title>.blocks.xml` beside the original snapshot only for documents this conversation has read; repeated reads refresh the same path only through the original, identity-checked file handle, and the XML reuses `maxReceiveFileBytes`. Identity safety wins over atomic replacement here: a swapped parent or same-size replacement cannot redirect the content, but refresh is not promised atomic. `comment_on_doc` accepts the same set and always includes an anchor, never a whole-document comment. Comments are not approved one by one. Aggregate text is capped at 10,000 Unicode code points per comment, and `maxDocCommentsPerTurn` defaults to 20 before resetting on the next turn. If either configuration or the comment capability is off, neither tool is registered and both appear in presence as unavailable.
