...
| `/put <path> [--into <document link>]` | Create a Feishu document from a workspace artifact, or append to one |
...
| `/doc list` | List open document-comment conversations (registrar DM only) |
| `/doc reset <document link>` | Reset one document's comment context (registrar DM only) |
...
- One group holds content for at most three agent-owned outbound operations, shared by `send_file` and `send_doc`. The actual content is frozen before an approval can be requested, so a fourth is refused before its content is read and the model is told to wait. The number is not configurable: raising it buys back the memory risk and the approval fatigue together.
- The channel no longer reads cloud-document bodies or lands snapshots automatically. Document links in chats and comments are pointers for the agent, which reads content itself through lark-cli when needed.
- `sendDocs` is on by default: `send_doc` creates a Feishu document from a workspace Markdown artifact. A new document in a direct chat grants only the initiating user; in a group it is not created until the group approves, then grants that group. The model tool no longer accepts `into`; a human's explicit `/put --into <link>` may still append without changing the target document's audience.
- If document creation succeeds but adding the collaborator fails, the channel still returns the real link and says that only the bot can open it — so nobody can share it by hand, because opening it is the first step of sharing — and points at the workspace copy to publish again once the scope is granted. The token goes to the operator console, which is the only way anyone can deal with a document no person can open. An existing document is never presented as wholly successful or wholly failed.
- A turn stopped mid-flight never turns a landed write into a failure: the channel reports the real link, says the turn was cancelled but the write did land and must not be resent, and records it on the operator console. A cancellation that catches a request still on the wire says exactly that — whether the platform applied it is unknown, so check before resending. "Failed" is the one word that makes a person send the same content again, appending it twice.
- The only model-visible document capability is the `send_doc` write tier, probed at startup through `application.v6.scope.list`. It remains optimistic when probing is refused, times out, or uses an unverified scope map; a real `permission_violations` response corrects it. `commentDocs` is a conversation-surface switch, not a model-tool capability tier.
- When a real document call returns `permission_violations`, the corresponding tools disappear from the next agent created. `docAuthorizeOnDemand` defaults on: the app-changing authorization request includes the exact missing scopes and the `drive.notice.comment_add_v1` event subscription, and is sent privately to the recorded registrar, falling back to the source chat only with an explicit warning. A post-scan probe reports what actually changed; ignored `addons` or pending approval is never presented as success.
- `commentDocs` defaults on. An @mention in a document comment opens a document-scoped conversation; the channel combines the quote, question, document link, surface-location note, and untrusted-data warning, then posts the terminal answer back to the triggering thread. First contact privately notifies the registrar and warns when the document is visible outside the tenant. At most 100 document agents stay live; LRU eviction releases an agent without deleting durable history. The surface registers no model-facing document tools: the agent reads content through lark-cli, and the single terminal reply is the only way back into the thread. Replies over 10,000 Unicode code points are truncated before sending with a visible notice.
- In a direct chat with the bot, the recorded app registrar can use `/doc list` to inspect live document-comment conversations and `/doc reset <document link>` to start one document with clean context. The reset generation persists through settings, so a plugin restart does not restore the old context. Groups and other users cannot read or reset this bridge-wide inventory. `/status` shows the latest comment-event time, or explicitly says that no event has ever arrived and points at the event subscription.
