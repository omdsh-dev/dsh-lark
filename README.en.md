...
- If document creation succeeds but adding the collaborator fails, the channel still returns the real link, says that only the bot can open it and asks for manual sharing, and reports the partial failure to the operator. An existing document is never presented as wholly successful or wholly failed.
- Document capabilities are probed independently at startup through `application.v6.scope.list`. `receiveDocs`, `sendDocs`, and `commentDocs` default on, but each capability lights only when all of its tenant scopes are granted; `/status` shows read, write, comment, and exact missing scopes.
